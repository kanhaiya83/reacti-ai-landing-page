...
const AdminPage = () => {
  ...
  const content = isLoggedIn ? (
    <AdminPanel couponsData={couponsData} setCouponsData={setCouponsData}/>
  ) : (
...
const AdminPanel = ({ setIsLoggedIn, couponsData ,setCouponsData}) => {
  ...
  return (
    <div className="w-full px-[5%] py-8">
      ...
      <div className="grid grid-cols-[repeat(24,1fr)] w-full bg-slate-900 text-center mt-10">
        ...
        {couponsData.map((c, i) => {
          return <RowItem data={c} index={i} key={i} setCouponsData={setCouponsData}/>;
        })}
      </div>
...
const RowItem = ({ data, index,setCouponsData }) => {
  ...
  const handleRevoke=async ()=>{
    const res = await fetch(
      import.meta.env.VITE_SERVER_URL + "/admin/"+(data.isRevoked ? "activate" : "revoke")+"/"+data.key
    );
    const response = await res.json();
    if (response.success) {
      let dataArr = [];
  ...
  return (
    <>
      ...
      <div className=" col-span-3 p-2">
        <h1>{data.isRevoked  ?  <span className="bg-red-500 p-1 rounded">Revoked</span> : <span className="bg-green-500 p-1 rounded">Active</span> }</h1>
      </div>
      <div className=" col-span-3 p-2">
        <h1>{data.date ? new Date(data.date).toDateString():new Date().toDateString()}</h1>
      </div>
      <div className=" col-span-2 p-2">
        ...
        >
          ...
          {open && <div className="absolute left-full top-[50%] text-black bg-slate-300 z-10 rounded">
            <button className="px-2 w-full py-2 border-b border-slate-500" onClick={handleRevoke} >{data.isRevoked ? "Activate": "Revoke"}</button>
            <button className="px-2 w-full py-2"  onClick={handleDownload}>Download</button>
          </div>}
        </button>
      </div>
    </>
